<template>
  <div class="Technology">
    ...
    <h2>Technology</h2>
    <p>Use this to add a new technology. Fill out as much or as little as you like.</p>
    ...
    <button class="primary large saveButton" v-on:click="addEntity">Save Spell!</button>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      const currentTechnology = values.technology.filter((technology) => {
        if (technology._id === this.$route.params.id) {
          return true;
        }
      });

      this.currentId = currentTechnology[0]._id;

      this.overviewValues = currentTechnology[0].value.overview;
      this.completeValues.overview = {...currentTechnology[0].value.overview};

      this.historyValues = currentTechnology[0].value.history;
      this.completeValues.history = {...currentTechnology[0].value.physical};

      this.mediaValue = currentTechnology[0].value.media;
  ...
  methods: {
    ...
    addEntity() {
      ...
      axios({
        ...
        this.dropdownText = 'Your Technology Has Been Added!';
