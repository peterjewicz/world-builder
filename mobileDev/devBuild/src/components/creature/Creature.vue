<template>
  <div class="Creature">
    ...
    <h2>Creature</h2>
    <p>Create a new creature, monster, or species. Fill out as much or as little as you like.</p>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      const currentCreature = values.creature.filter((city) => {
        if (city._id === this.$route.params.id) {
          return true;
        }
      });

      this.currentId = currentCreature[0]._id;
  ...
  methods: {
    ...
    addEntity() {
      ...
      axios({
        ...
        this.dropdownText = 'Your Creature Has Been Saved!';
