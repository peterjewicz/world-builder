<template>
  <div class="Creature">
    ...
    <p>Use this to create a location that doesn't fit into the other categories such as a fort or site of a battle. Fill out as much or as little as you like.</p>
    ...
    <button class="primary large saveButton" v-on:click="addEntity">Save Creature!</button>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      const currentInterest = values.pointofinterest.filter((interest) => {
        if (interest._id === this.$route.params.id) {
          return true;
        }
      });
      this.currentId = currentInterest[0]._id;

      this.overviewValues = currentInterest[0].value.overview;
      this.completeValues.overview = {...currentInterest[0].value.overview};

      this.historyValues = currentInterest[0].value.history;
      this.completeValues.history = {...currentInterest[0].value.physical};

      this.mediaValue = currentInterest[0].value.media;
