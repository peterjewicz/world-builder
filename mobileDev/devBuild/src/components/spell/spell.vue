<template>
  <div class="Spell">
    ...
    <h2>Spell</h2>
    <p>Use this to create a new spell or magic effect. Fill out as much or as little as you like.</p>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      const currentSpell = values.spell.filter((spell) => {
        if (spell._id === this.$route.params.id) {
          return true;
        }
      });

      this.currentId = currentSpell[0]._id;

      this.overviewValues = currentSpell[0].value.overview;
      this.completeValues.overview = {...currentSpell[0].value.overview};

      this.historyValues = currentSpell[0].value.history;
      this.completeValues.history = {...currentSpell[0].value.physical};

      this.mediaValue = currentSpell[0].value.media;
