...
The `home-routes` handler in the `worldbuilder.routes.home` namespace
...
The `home-page` function will in turn call the `worldbuilder.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `worldbuilder.handler` namespace:
...
Request middleware functions are located under the `worldbuilder.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `worldbuilder.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `worldbuilder.core` in some other namespace.
